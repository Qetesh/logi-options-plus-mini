...
Write-Host "$(Get-Date) | Detecting region via https://cloudflare.com/cdn-cgi/trace"
try {
    $traceResponse = Invoke-RestMethod -Uri "https://cloudflare.com/cdn-cgi/trace" -TimeoutSec 10
    if ($traceResponse -match "loc=CN") {
        Write-Host "$(Get-Date) | Detected region: China, using CN download URL"
        $selectedDownloadUrl = $downloadUrlCN
    } else {
        Write-Host "$(Get-Date) | Detected region: International, using default download URL"
        $selectedDownloadUrl = $downloadUrl
    }
} catch {
    Write-Host "$(Get-Date) | Failed to detect region, using default download URL"
    $selectedDownloadUrl = $downloadUrl
}

Write-Host "$(Get-Date) | Downloading $appName Installer from: $selectedDownloadUrl"
...
Write-Host "0. quiet:                 Install the application silently without UI."
